// OPS-04 Article A: 실제 WordPress 블로그(설명형, 2번째 사례) — 제한
// 운영 확대. 실제 Supabase DB / 실제 Anthropic API / 실제 WordPress
// 사이트(Draft만). 공개 게시는 호출하지 않는다.
import "./load-env";
...
const LABEL = "ops-04-article-a";
const TOPIC_TITLE = "실내 공기질 관리와 미세먼지 대응 방법";
const SOURCE_URLS = [
  "https://ko.wikipedia.org/wiki/미세먼지",
  "https://ko.wikipedia.org/wiki/실내공기질",
...
describe("OPS-04 Article A: WordPress blog(실내 공기질) 실제 운영", () => {
  ...
  it("1) 테마 생성 + 출처 3건 등록(실제 fetch + AI 요약)", async () => {
    ...
    const theme = await createTheme({
      title: TOPIC_TITLE,
      description: "미세먼지가 건강에 미치는 영향과 실내 공기질을 실제로 개선하는 방법을 출처 기반으로 설명하는 블로그 글.",
      keywords: ["미세먼지", "실내공기질", "공기청정기", "환기"],
      language: "ko",
    });
    themeId = theme.id;
    ...
    for (const url of SOURCE_URLS) {
      ...
      console.log(`[${LABEL}] source ${source.id} fetch status=${fetchResult.status} contentLength=${fetchResult.rawContent?.length ?? 0}`);
  ...
  it("6) 자동 검토(quality gate) 실행 + 필요 시 자동 수정 + fact-grounding 관찰", async () => {
    ...
    console.log(
      `[${LABEL}] review counts=${JSON.stringify(review.counts)} riskLevel=${review.riskLevel} confirmationCount=${userFacing.confirmationCount} fact_grounding=${factGroundingItem ? JSON.stringify(factGroundingItem) : "not_run"}`
    );
    ...
    if (post.qualityStatus === "needs_revision") {
      ...
      mark("6b_auto_fix", t1);
      console.log(`[${LABEL}] auto_fix success=${autoFix.success} finalState=${autoFix.finalState} changes=${JSON.stringify(autoFix.changesApplied)}`);
      post = (await getSocialPostById(socialPostId))!;
    }
    console.log(`[${LABEL}] autoFixTriggered=${autoFixTriggered}`);
    ...
    console.log(`[${LABEL}] canApprove=${gate.canApprove} reason=${gate.reason}`);
  ...
  it("7) 승인 + WordPress 게시 준비 일괄 실행(실제 Draft 생성)", async () => {
    ...
    console.log(`[${LABEL}] steps=${JSON.stringify(result.steps)}`);
    expect(result.success).toBe(true);
  }, 180_000);

  it("8) Publish guard 확인", async () => {
    const guardResult = await runPlatformPublishingGuard(socialPostId);
    console.log(`[${LABEL}] guard status=${guardResult.result?.status} score=${guardResult.result?.score} blockedReasons=${JSON.stringify(guardResult.result?.blockedReasons)}`);
