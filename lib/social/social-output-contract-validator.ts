...
/**
 * 개인정보 노출 의심 패턴(주민등록번호/휴대전화 형식).
 * 휴대전화는 "010-"로 시작하는 번호만 의심한다 — wordpress_blog는
 * 정책/지원사업 안내 글 특성상 제주도청(064-710-4252), 복지로,
 * 콜센터(1588-XXXX 등) 같은 **공개된 기관 문의처**를 정당하게 포함해야
 * 하는데(E-E-A-T/실행 가능한 다음 단계), 이런 지역번호/대표번호까지
 * `\d{3}-\d{3,4}-\d{4}` 패턴으로 잡으면 정상적인 기관 연락처까지
 * "개인정보 노출 의심"으로 차단하는 오탐이 발생한다.
 */
