...
const CAPABILITIES: Record<SocialPlatform, PlatformApiCapability> = {
  ...
  opinion_column: {
    ...
    requiresAppReview: false,
    currentMode: "manual_export",
    publishEnabledFlagName: "OPINION_COLUMN_API_PUBLISH_ENABLED",
    notes: "칼럼도 특정 CMS API와 연동하지 않으므로 manual export를 기본 경로로 유지한다.",
    warnings: ["칼럼은 연동된 CMS API가 없습니다 — 실제 게시(배포)는 manual export를 사용하세요."],
  },
  wordpress_blog: {
