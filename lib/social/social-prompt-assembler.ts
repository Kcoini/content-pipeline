...
export function assembleSocialWritingPrompt(context: SocialWritingContext): AssembledSocialPrompt {
  ...
  const systemPrompt = [
    ...
    // OPS-02A: 실제 운영 파일럿(threads+story tone)에서 반복 재현된 JSON
    // 파싱 실패의 정확한 원인 — 모델이 본문 안에 대화/인용을 표현할 때
    // 큰따옴표(")를 그대로 쓰고 이스케이프(\")를 빠뜨려 JSON 문자열이
    // 깨졌다(예: "post_body": "...가장 먼저 물었던 건 "배터리 얼마나
    // 가냐"는 거였어요..."). 특정 tone/platform만의 문제가 아니라 대화체
    // 인용이 들어갈 수 있는 모든 조합에서 재현될 수 있어, 여기(공통
    // system prompt)에서 한 번에 예방한다 — 파싱을 느슨하게 만들어
    // 손상된 JSON을 억지로 복구하지 않고, 애초에 깨지지 않게 한다.
    "문장 안에서 대화나 인용을 표현할 때는 큰따옴표(\") 대신 작은따옴표(')나 낫표(「」)를 쓰세요 — JSON 문자열 값 안에서 큰따옴표를 이스케이프 없이 쓰면 파싱이 실패합니다. 큰따옴표를 꼭 써야 한다면 반드시 \\\"로 이스케이프하세요.",
    "",
    "## Platform 규칙",
    platformPrompt,
    "## Tone 규칙",
