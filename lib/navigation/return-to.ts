...
// Phase 3-23-2: 대시보드에서 실행한 액션(플랫폼별 글 생성 등)도 대시보드
...
// rewrite/performance)와 `/dashboard` 루트뿐이다. 그 외 모든 값(외부 URL,
// protocol-relative URL, javascript:/data: 스킴, `/dashboard`의 하위
// 경로 등)은 안전하지 않다고 간주하고 fallback을 반환한다 — 이 프로젝트가
// 임의의 open redirect를 허용하지 않기 위함이다.
