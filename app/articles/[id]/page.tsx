      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-6">
        ...
        */}
        ...
        <section className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
          ...
          {!contentSummary ? (
            <p className="mt-2 text-xs text-zinc-500">콘텐츠 요약을 불러올 수 없습니다.</p>
          ) : (
            <>
              <div className="mt-2 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
                <div className="rounded border border-blue-200 bg-blue-50 p-2">
                  <p className="text-[11px] text-blue-700">블로그 글</p>
                  <p className="font-semibold text-blue-700">{contentSummary.blogPostCount}</p>
                </div>
                <div className="rounded border border-purple-200 bg-purple-50 p-2">
                  <p className="text-[11px] text-purple-700">SNS/커뮤니티 글</p>
              ...
              <p className="mt-2 text-xs text-zinc-500">
                게시 완료: {contentSummary.postedCount}개 · metrics 측정됨: {contentSummary.metricsMeasuredCount}개 · metrics 미입력:{" "}
                {contentSummary.metricsMissingCount}개 · 재게시 추천: {contentSummary.recommendedRewriteCount}개
              </p>
              <p className="mt-1 text-[11px] text-zinc-400">최근 콘텐츠 업데이트: {new Date(contentSummary.latestUpdatedAt).toLocaleString("ko-KR")}</p>
            </>
          )}
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            <Link
              ...
              className="rounded border border-blue-300 bg-blue-50 px-3 py-1.5 font-medium text-blue-700 hover:bg-blue-100"
            >
              블로그 글쓰기 관리 →
            </Link>
            <Link
              href={`/articles/${article.id}/social`}
              className="rounded border border-purple-300 bg-purple-50 px-3 py-1.5 font-medium text-purple-700 hover:bg-purple-100"
            >
              SNS/커뮤니티 글쓰기 관리 →
            </Link>
            <Link
              href={`/articles/${article.id}/rewrite`}
              className="rounded border border-indigo-300 bg-indigo-50 px-3 py-1.5 font-medium text-indigo-700 hover:bg-indigo-100"
            >
              Rewrite 관리 →
            </Link>
            <Link
              href={`/articles/${article.id}/performance`}
              className="rounded border border-amber-300 bg-amber-50 px-3 py-1.5 font-medium text-amber-700 hover:bg-amber-100"
            >
              성과 보기 →
            </Link>
          </div>
