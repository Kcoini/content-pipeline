...
## 본문 관련 버튼은 항상 한 줄, 같은 순서로 배치한다 (Phase 4-27)

`SocialPostBodyPanel`(모든 글 카드가 공유하는 본문 확인 컴포넌트)의
[본문 복사]/[전체 보기]·[본문 접기]/[본문 수정] 버튼은 `PostBodyActionRow`
(`components/social/post-body-action-row.tsx`) 하나로 모아 본문 텍스트
바로 아래 한 줄에 렌더링한다 — 이전에는 [본문 복사]가 라벨 옆(상단),
[전체 보기]가 본문 텍스트 아래, [본문 수정]이 맨 아래로 서로 다른
위치에 흩어져 있었다.

- **순서**: 항상 [본문 복사] → [전체 보기]/[본문 접기](필요할 때만) →
  [본문 수정](editable일 때만) 순서다.
- **[전체 보기] 표시 조건**: 본문이 1,200자 이하면 렌더링 자체를
  하지 않는다(비활성화가 아니라 미표시) — `needsExpandableCollapse()`
  (`components/social/expandable-text.tsx`)로 판단한다. 1,200자를
  넘으면 표시하고, 펼친 상태에서는 라벨이 [본문 접기]로 바뀐다.
  클릭해도 상세 페이지로 이동하지 않고, 같은 카드 안에서 본문 텍스트
  (`ExpandableText`, `expanded`로 controlled)만 펼쳐지거나 접힌다.
- **[본문 복사]**: 항상 렌더링된다. 접힌 상태여도 화면에 보이는
  축약문이 아니라 `displayBody`(플랫폼별 우선순위로 계산된 게시용
  본문) 전체를 복사한다.
- **[본문 수정]**: `editable`(x처럼 threadItems 기반 플랫폼은 false)일
  때만 렌더링되고, 클릭하면 페이지 이동 없이 같은 카드 안에서
  textarea 편집 모드로 바뀐다(기존 Phase 4-15 동작 그대로 — 버튼
  위치만 통일했다).
- **레이아웃**: `flex flex-wrap items-center gap-2` — 데스크톱은 한
  줄, 좁은 화면(모바일)에서는 버튼이 자연스럽게 다음 줄로 넘어간다.
- `hideBodyWhenNotEditing`(wordpress_blog 탭 밖 공통 위치, Phase
  4-23)에서는 본문 텍스트 자체를 안 보여주므로 [전체 보기]도 항상
  미표시고, [본문 복사]/[본문 수정]만 한 줄에 남는다.

이 버튼 행은 `SocialPostBodyPanel`을 쓰는 모든 카드(WordPress
블로그/네이버 블로그/네이버 카페/X/Threads/Instagram/언론 기사/칼럼/
rewrite/대시보드 테이블)에 자동으로 적용된다 — 카드마다 버튼을 따로
배치하지 않는다.

## 자동 검토 "수정 필요" — 자동 수정 가능한 문제만 남으면 [자동 수정 후 재검토]가 기본 버튼 (Phase 4-28)

naver_blog/news_article/opinion_column 카드(wordpress_blog는 별도
탭에서 동일 원칙 적용)에서, 자동 검토가 "수정 필요"를 판단했을 때
남은 문제 전부가 auto_fixable + 실제 구현된 자동 수정기가 있는
항목뿐이면:

- "문제 확인하기"(기존 기본 버튼) 대신 [자동 수정 후 재검토]가
  primary(강조) 버튼이 된다 — "문제 확인하기"는 secondary로 내려간다
  (primary 버튼은 항상 하나만 강조).
- 버튼 위에 "자동으로 정리할 수 있는 항목 N개를 발견했습니다 ...
  새로운 사실이나 수치는 추가하지 않습니다" 안내를 보여준다.
- 출처/수치/기관명 확인이 필요한 문제가 하나라도 섞여 있으면 이
  승격은 일어나지 않는다 — 기존처럼 "문제 확인하기"가 기본이다.

글 생성 직후에도 같은 판단 기준(`hasOnlyImplementedAutoFixableIssues`)을
써서, 남은 문제가 전부 자동 수정 가능하면 사용자가 화면을 보기 전에
이미 자동 정리·재검토가 끝나 있다 —
