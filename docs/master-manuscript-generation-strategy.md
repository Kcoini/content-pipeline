...
- [`phase-4-8-master-manuscript-evidence-quality.md`](./phase-4-8-master-manuscript-evidence-quality.md) — 최상위 evidenceMap(강도/주의사항)/issues/readerMeaning/factType 추가, 마스터 원고 자체 자동 검토(`reviewMasterManuscript`), 플랫폼 글 생성 prompt에 근거 하이라이트 전달
- [`phase-4-9-master-manuscript-undefined-filter-fix.md`](./phase-4-9-master-manuscript-undefined-filter-fix.md) — 구버전 마스터 원고(evidenceMap 등 신규 필드 없음)를 읽을 때 발생한 "undefined.filter" 오류 수정: `normalizeMasterManuscript()`로 항상 완전한 배열/객체 shape을 보장
