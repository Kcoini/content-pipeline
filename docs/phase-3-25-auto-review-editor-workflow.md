...
- [`phase-4-5-content-type-review-separation.md`](./phase-4-5-content-type-review-separation.md) — 이 문서의 `summarizeAutoReview` 리포트에 "글 유형(platform)과 적용된 검토 기준"을 항상 먼저 보여주고, 글 유형과 본문 형태가 어긋나면 "확인 필요"로 안내하는 계층을 얹었다. 검토 엔진(`social-quality-gate.ts`)과 리포트 구조는 그대로다.
- [`phase-4-8-master-manuscript-evidence-quality.md`](./phase-4-8-master-manuscript-evidence-quality.md) — 이 문서(`summarizeAutoReview`, 플랫폼별 글 검토)와 별개로 마스터 원고 "자체"의 구조적 완성도를 검토하는 `reviewMasterManuscript()`를 새로 추가했다. 두 검토는 서로 다른 대상(마스터 원고 vs 플랫폼별 글)을 보며, 어느 쪽도 다른 쪽이나 사람의 최종 승인을 대체하지 않는다.
